...
st.set_page_config(page_title="A MACHINE LEARNING PROGRAM FOR PREDICTING THREE TYPES OF DISEASES",layout="wide", page_icon="👨‍🦰🤶")
...
kindey_disease_model = pickle.load(open(f'{working_dir}/saved_models/kindey.pkl','rb'))
...
with st.sidebar:
    selected = option_menu("A MACHINE LEARNING PROGRAM FOR PREDICTING THREE TYPES OF DISEASES", 
                ['Diabetes Prediction',
                 'Heart Disease Prediction',
                 'Kindey Disease Prediction'],
...
if selected == 'Kindey Disease Prediction':
    
    st.title("Kindey Disease Prediction using ML")
    ...
    if st.button("Kindey's Test Result"):
       ...
       peda_edema, aanemia]
        ...
        prediction = kindey_disease_model.predict([user_input])

        if prediction[0] == 1:
            kindey_diagnosis = "The person has Kindey's disease"
        else:
            kindey_diagnosis = "The person does not have Kindey's disease"
